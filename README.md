...
Published packages: **`vtools-core`** (library) and **`vtools`** (CLI). Both stay on the **same version** (`fixed` group in Changesets).
...
3. Ensure the npm package names `vtools` and `vtools-core` are available.
